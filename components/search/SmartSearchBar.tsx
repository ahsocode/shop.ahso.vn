...
export default function SmartSearchBar({
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/search/autocomplete?q=${encodeURIComponent(query)}&limit=${suggestionLimit}`
        );
  ...
  }, [query, searchHistory, showHistory, debounceMs, suggestionLimit]);
      ...
      {showSuggestions && suggestions.length > 0 && (
        ...
        >
          <div className="max-h-96 overflow-y-auto">
            {/* History header */}
            {showHistory && suggestions[0]?.type === "history" && (
